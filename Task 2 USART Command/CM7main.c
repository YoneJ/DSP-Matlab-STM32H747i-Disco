...
/* USER CODE BEGIN PV */
uint8_t rx_buffer[RX_BUFFER_SIZE]; // Buffer for received data
uint8_t rx_data;                   // Temporary storage for received byte
volatile bool command_received = false; // Flag to indicate a complete command is received
/* USER CODE END PV */
...
void process_command(uint8_t *command)
{
    ...
    {
        HAL_GPIO_WritePin(GPIOI, LED1_Pin, GPIO_PIN_RESET); // Turn on green LED

    }
    else if (strcmp((char *)command, "LED1 OFF") == 0)
    {
        HAL_GPIO_WritePin(GPIOI, LED1_Pin, GPIO_PIN_SET); // Turn off green LED
    }
    else if (strcmp((char *)command, "LED2 ON") == 0)
    {
        HAL_GPIO_WritePin(GPIOI, LED2_Pin, GPIO_PIN_RESET); // Turn on orange LED
    }
    else if (strcmp((char *)command, "LED2 OFF") == 0)
    {
        HAL_GPIO_WritePin(GPIOI, LED2_Pin, GPIO_PIN_SET); // Turn off orange LED
    }
